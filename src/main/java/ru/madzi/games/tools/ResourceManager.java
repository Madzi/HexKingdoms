...
public class ResourceManager {
    ...
    public Texture getTexture(String resName) throws IOException {
        Texture tex = (Texture) table.get(resName);
        if (tex == null) {
            tex = getTexture(resName, GL_TEXTURE_2D, GL_RGBA, GL_LINEAR, GL_LINEAR);
            table.put(resName, tex);
        }
        return tex;
    }

    public Texture getTexture(String resName, int target, int dstPixelFormat, int minFilter, int magFilter) throws IOException {
        int srcPixelFormat = 0;
        int textureID = createTextureID();
        Texture texture = new Texture(target, textureID);
        glBindTexture(target, textureID);
        BufferedImage bufferedImage = loadImage(resName);
    ...
    private BufferedImage loadImage(String resName) throws IOException {
        URL url = ResourceManager.class.getClassLoader().getResource(resName);
        if (url == null) {
            throw new IOException("Cannot find: " + resName);
        }
        BufferedImage bufferedImage = ImageIO.read(new BufferedInputStream(getClass().getClassLoader().getResourceAsStream(resName)));
